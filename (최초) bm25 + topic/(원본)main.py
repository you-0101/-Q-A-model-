...
if __name__ == '__main__':
    ...
    for epoch in range(1, 1 + args.total_epoch):
        ...
        for batch in range(batch_len):
            ...
            print('[E%d] L %.6f | RQ %.6f | RP %.6f | RN %.6f | DP %.6f | DN %.6f (%d/%d) [%s]' % (epoch, loss.item(), args.recon_alpha * recon_q.item(), args.recon_alpha * recon_p.item(), args.recon_alpha * recon_n.item(), args.prior_alpha * dist_p.item(), args.prior_alpha* dist_n.item(), batch + 1, batch_len, format_time(start, end, batch_len, batch + 1)), end='\r', flush=True)x
